...
public class SemanticCheckVisitor implements Visitor {
	...
	public SemanticCheckVisitor(PrintWriter outfile, List<ClassDecl> classDeclList) {
	...
	public void visit(Program program) {
		...
		for (ClassDecl classdecl : program.classDecls()) {
			...
			if (classdecl.superName() != null && classdecl.superName().equals("Main")) {
	...
	public void visit(MethodDecl methodDecl) {
		...
		for (var formal : methodDecl.formals()) {
			if (varNames.contains(formal.name())) {
				printError(" 24. Variable redeclaration is forbidden");
			} else {
				varNames.add(formal.name());
			}
		}
		for (var varDecl : methodDecl.vardecls()) {
			if (varNames.contains(varDecl.name())) {
				printError(" 24. Variable redeclaration is forbidden");
			} else {
				varNames.add(varDecl.name());
			}
		}
